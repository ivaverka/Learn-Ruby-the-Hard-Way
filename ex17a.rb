...
# we open the from_file. We just open it, we are getting into it - that's it.
in_file = open(from_file)
# now we can do anything we want with this file once it was opened. Here we want to read it.
# to do that we use the in_file variable because it contains everything that will help us read the file (first we need to open it).
indata = in_file.read
